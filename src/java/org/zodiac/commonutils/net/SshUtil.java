...
public class SshUtil {
	...
	public static enum AuthType {
		AUTH_PASSWORD, AUTH_PRIVATE_KEY;
	}

	public class Builder {

		private AuthType authType = AuthType.AUTH_PASSWORD;
	...
	public class SshConnector {
		...
		private SshConnector(String host, int port, String user, String prvKeyFile, String pubKeyFile,
			...
			if (null == session || !session.isConnected()) {
				if (authTpye == AuthType.AUTH_PRIVATE_KEY) {
					try {
						...
						logger.error(String.format("Error when get Session on %s %s", host,
								e.getCause() + "\n" + e.getMessage()), e);
					}
					if (null != session && session.isConnected()) {
					...
					try {
						...
						logger.error(String.format("Error when Connect to host: %s with port: %d, user: %s", host, port,
								user));
						e.printStackTrace();
					}
				}
				if (null != session && session.isConnected()) {
					...
					logger.error(String.format("Connect to %s Failed !", host));
				}
				return session;
			} else {
				return session;
		 ...
		 * @param maxOutput  最大输出字符数
		 */
		public void execCmd(String cmd, boolean backGround, int maxOutput, InputStreamHandler handler) {
			session = getSession();
			...
			try {
				...
				logger.error(String.format("Error when get Chanel on host: %s with port: %d, user: %s. %s", host, port,
						user, e.getCause() + "\n" + e.getMessage()));
				e.printStackTrace();
			}
			...
			try {
				...
				logger.error(String.format("Error when Connect to Channel on host: %s with port: %d, user: %s. %s",
						host, port, user, e.getCause() + "\n" + e.getMessage()));
			...
			try {
				...
				logger.error(String.format(
						"Error when Get OutputStream of command: %s on host: %s with port: %d, user: %s. %s", cmd, host,
						port, user, e.getCause() + "\n" + e.getMessage()));
				closeChannel(channel);
				e.printStackTrace();
			}
			assert null != in;
		...
		public void execCmd(String cmd, boolean backGround, int maxOutput) {
			execCmd(cmd, backGround, maxOutput, new InputStreamHandler() {
				@Override
				public void handle(InputStream inputStream) {
					...
					try {
						...
						logger.error(String.format("Error when Read from commamd: %s Output! %s", cmd,
								e.getCause() + "\n" + e.getMessage()), e);
						e.printStackTrace();
					}
				}
		...
		public boolean upload(String sourceFile, String destFile, boolean overwrite) {
			...
			try {
				...
				logger.error(String.format("Error when upload %s to %s %s", sourceFile, destFile,
						e.getCause() + "\n" + e.getMessage()), e);
			}
			return ret;
		}
		...
		public void upload(File sourceFile, String destFile, boolean overwrite) throws IOException {
			...
			if (sourceFile.isDirectory()) {
				...
				logger.info(String.format("Uploaded %s to %s!", sourceFile.getAbsolutePath(), destFile));
			}
		}
		...
		private void upload(String source, String dest) {
			...
			if (sourceFile.isDirectory()) {
				...
				try {
					...
					logger.error(String.format("Error when upload %s to %s %s", source, dest,
							e.getCause() + "\n" + e.getMessage()));
					e.printStackTrace();
				}
			}
		...
		private void upload(byte[] content, String destFile, int mode) {
			...
			if (!isDir(destFile)) {
				...
				try {
					...
					logger.error(String.format("Error when create OutputStream on path %s %s", destFile,
							e.getCause() + "\n" + e.getMessage()), e);
					e.printStackTrace();
				} catch (IOException e) {
					logger.error(String.format("Error when write %s to %s %s", bufferSize + "bytes", destFile,
							e.getCause() + "\n" + e.getMessage()), e);
					e.printStackTrace();
				} finally {
					try {
...
//                    !attrs.isBlk() && !attrs.isChr() && !attrs.isFifo() && !attrs.isReg() && !attrs.isSock() && !attrs.isLink();
					...
					if (isDir(x)) {
						if (!hasChild(x)) {
							try {
								...
								logger.error(String.format("Error when delet dir %s %s", x,
										e.getCause() + "\n" + e.getMessage()), e);
							}
						} else {
							delete(x);
						...
						try {
							...
							logger.error(String.format("Error when delet file %s %s", x,
									e.getCause() + "\n" + e.getMessage()), e);
							ret.set(false);
						}
					}
		...
		public void rename(String source, String dest) {
			...
			try {
				...
				logger.error(String.format("Error when rename %s to %s  %s", source, dest,
						e.getCause() + "\n" + e.getMessage()), e);
				e.printStackTrace();
			}
		}
		...
		private ChannelSftp getChannelSftp() {
			if (null == channelSftp || channelSftp.isEOF() || channelSftp.isClosed()) {
				...
				try {
					...
					logger.error(String.format(
							"Error when Connect to host: %s with port: %s, user: %s, password: %s, timeout: %s", host,
							port, user, "***", timeout), e);
					e.printStackTrace();
				}
				return channelSftp;
	...
	public class MySftpProgressMonitor implements SftpProgressMonitor {
		...
		public boolean count(long count) {
			...
			n += 1;
			if (n % m == 0) {
				logger.info(String.format("source file %s total %s, transformed %s, percentage %s", src, total,
						transformed, percent * 100));
			}
			return true;
		}
